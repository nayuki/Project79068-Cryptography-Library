 ...
 public UnicodeString(String str){
  ...
  for(int i=0;i<str.length();j++){
   ...
   if(c<0xD800||c>=0xE000){ // Not a surrogate
    ...
    if(c>=0xDC00&&c<0xE000)throw new IllegalArgumentException("Malformed string"); // Low surrogate
    int tp=c;
    i++;
    if(i==str.length())throw new IllegalArgumentException("Malformed string");
    c=str.charAt(i);
    if(c<0xDC00||c>=0xE000)throw new IllegalArgumentException("Malformed string"); // Not a low surrogate
 ...
 */
 ...
 public UnicodeString(int[] chars,int off,int len){
  ...
  for(int i=0;i<len;i++){
   int c=chars[off+i];
   if(c<0||c>0x10FFFF)throw new IllegalArgumentException();
   this.chars[i]=c;}}
 ...
 public int find(int ch,int start){
  ...
  if(ch<0||ch>0x10FFFF)throw new IllegalArgumentException("Invalid Unicode code point");
  for(;start<chars.length;start++){
   if(chars[start]==ch)return start;}
  return -1;}
 ...
 public boolean equals(Object obj){
  ...
  for(int i=0;i<chars.length;i++){
   if(chars[i]!=str.chars[i])return false;}
  return true;}

 public int hashCode(){
  HashCoder h=HashCoder.newInstance();
  for(int i=0;i<chars.length;i++)h.add(chars[i]);
  return h.getHashCode();}

 public int compareTo(UnicodeString str){
  for(int i=0;i<chars.length&&i<str.chars.length;i++){
   if(chars[i]!=str.chars[i])return chars[i]-str.chars[i];}
