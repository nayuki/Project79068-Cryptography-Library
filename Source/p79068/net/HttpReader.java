...
public class HttpReader{
 ...
 public HttpRequest readRequest() throws IOException{
  ...
  while(true){
   ...
   if(temp.length==1)request.addHeader(temp[0],null);
   else request.addHeader(temp[0],temp[1]);}
/* Read message body */
  return request;}
 ...
 private HttpResponse readResponse(boolean allowEntity) throws IOException{
  ...
  while(true){
   ...
   if(line.equals(""))break;
   String[] temp=splitHeaderLine(line);
   if(temp.length==1)response.addHeader(temp[0],null);
   else response.addHeader(temp[0],temp[1]);}
/* Read message body */
  return response;}
 ...
 protected String[] splitHeaderLine(String str){
  if(str.indexOf(':')==-1)return new String[]{str};
  else{
