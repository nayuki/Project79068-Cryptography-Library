 ...
 */
 public static int extractBits(int x,int bitOffset,int bitLength){
  ...
  if(bitLength==32)return x; // bitOffset = 0, or else an exception was already thrown.
 ...
 */
 ...
 public static byte[] toBytesBigEndian(int[] ain){
  ...
  for(int i=0;i<aout.length;i++)aout[i]=(byte)(ain[i/4]>>>((3-i%4)*8));
  return aout;}

 public static byte[] toBytesLittleEndian(int[] ain){
  byte[] aout=new byte[ain.length*4];
  for(int i=0;i<aout.length;i++)aout[i]=(byte)(ain[i/4]>>>(i%4*8));
  return aout;}

 public static int[] fromBytesBigEndian(byte[] ain){
  if(ain.length%4!=0)throw new IllegalArgumentException("Length is not multiple of 4");
  int[] aout=new int[ain.length/4];
  for(int i=0;i<ain.length;i++)aout[i/4]|=(ain[i]&0xFF)<<((3-i%4)*8);
  return aout;}

 public static int[] fromBytesLittleEndian(byte[] ain){
  if(ain.length%4!=0)throw new IllegalArgumentException("Length is not multiple of 4");
  int[] aout=new int[ain.length/4];
  for(int i=0;i<ain.length;i++)aout[i/4]|=(ain[i]&0xFF)<<(i%4*8);
  return aout;}
