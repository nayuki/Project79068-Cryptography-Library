...
if(!new UnsignedBigInteger(a).GCD(new UnsignedBigInteger(b)).toString().equals(
"0x"+BigInteger.valueOf(a).gcd(BigInteger.valueOf(b)).toString(16)

))System.out.println(a+" "+b);
k++;
if(k%10000==0)System.out.print("\r"+k);
if(false)break;
}
...
if(true)return;
 ...
 public UnsignedBigInteger(byte[] value){ // The array is in big-endian
  ...
  int j=0;
  for(;i>=1;i-=2,j++)digit[j]=(short)(value[i-1]<<8|value[i]&0xFF);
  if(i==0)digit[j]=(short)(value[i]&0xFF);
  digit=trim(digit);}
 ...
 public UnsignedBigInteger add(UnsignedBigInteger val){
  ...
  if(digit.length>=val.digit.length){
   ...
   digitx=val.digit;
   digity=digit;}
  if((digitx[digitx.length-1]&0x8000)==0&&(digitx.length>digity.length||(digity[digity.length-1]&0x8000)==0))digitz=new short[digitx.length]; // No extra carry possible
  else digitz=new short[digitx.length+1];
  int carry=0;
  int i=0;
  ...
  for(;i<digitx.length;i++){
   ...
   digitz[i]=(short)carry;
   carry>>>=16;}
  if(carry!=0)digitz[i]=(short)carry;
 ...
 public UnsignedBigInteger subtract(UnsignedBigInteger val){
  ...
  if(digitx.length<digity.length)return null;
 ...
 public UnsignedBigInteger and(UnsignedBigInteger val){
  ...
  for(int i=0;i<digity.length;i++)digitz[i]=(short)(digitx[i]&digity[i]);
  return new UnsignedBigInteger(digitz);}
 ...
 public UnsignedBigInteger or(UnsignedBigInteger val){
  ...
  int i=0;
  for(;i<digity.length;i++)digitz[i]=(short)(digitx[i]|digity[i]);
  for(;i<digitx.length;i++)digitz[i]=digitx[i];
  return new UnsignedBigInteger(digitz);}
 ...
 public UnsignedBigInteger shiftRight(int shift){
  ...
  if(shift>=digit.length*16)return ZERO;
  short[] digitz;
  if(shift%16==0){
   if(shift==0)return this;
   digitz=new short[digit.length-shift/16];
 ...
 public UnsignedBigInteger GCD(UnsignedBigInteger val){
  ...
  while((x.digit[0]&1)==0&&(y.digit[0]&1)==0&&(x.digit.length>1||x.digit[0]!=0)&&(y.digit.length>1||y.digit[0]!=0)){ // x and y are even but non-zero
   ...
   y=y.shiftRight(1);}
  while(true){
   if(x.digit.length==1&&x.digit[0]==0)return y.shiftLeft(pow2); // If x is 0
   else if(y.digit.length==1&&y.digit[0]==0)return x.shiftLeft(pow2); // If y is 0
   else if((x.digit[0]&1)==0)x=x.shiftRight(1); // x is even
   else if((y.digit[0]&1)==0)y=y.shiftRight(1); // y is even
   else if(x.compareTo(y)>=0)x=x.subtract(y).shiftRight(1); // x and y are odd, x >= y
   else y=y.subtract(x).shiftRight(1);}} // x and y are odd, y > x


 public boolean equals(Object val){
  if(super.equals(val))return true;
  if(!(val instanceof UnsignedBigInteger))return false;
  short[] digity=((UnsignedBigInteger)val).digit;
  if(digit.length!=digity.length)return false;
  for(int i=0;i<digit.length;i++){
   if(digit[i]!=digity[i])return false;}
  return true;}

 public int compareTo(Object val){
  short[] digity=((UnsignedBigInteger)val).digit;
  if(digit.length!=digity.length)return digit.length-digity.length;
  for(int i=digit.length-1;;i--){
   if(digit[i]!=digity[i]||i==0)return (digit[i]&0xFFFF)-(digity[i]&0xFFFF);}}


 public String toString(){
  StringBuffer sb=new StringBuffer("0x");
  sb.append(Integer.toString(digit[digit.length-1]&0xFFFF,16));
  for(int i=digit.length-2;i>=0;i--)sb.append(hexdigit[(digit[i]>>>12)&0xF]).append(hexdigit[(digit[i]>>>8)&0xF]).append(hexdigit[(digit[i]>>>4)&0xF]).append(hexdigit[digit[i]&0xF]);
  return sb.toString();}
 ...
 private static short[] trim(short[] ain){
  if(ain.length==0)return new short[]{0};
  int i;
  for(i=ain.length-1;i>=1&&ain[i]==0;i--);
  if(i==ain.length-1)return ain;
