 ...
 */
 public static long extractBits(long x,int bitOffset,int bitLength){
  ...
  if(bitLength==64)return x; // bitOffset = 0, or else an exception was already thrown.
 ...
 */
 ...
 public static byte[] toBytesBigEndian(long[] ain){
  ...
  for(int i=0;i<aout.length;i++)aout[i]=(byte)(ain[i/8]>>>((7-i%8)*8));
  return aout;}

 public static byte[] toBytesLittleEndian(long[] ain){
  byte[] aout=new byte[ain.length*8];
  for(int i=0;i<aout.length;i++)aout[i]=(byte)(ain[i/8]>>>(i%8*8));
  return aout;}

 public static long[] fromBytesBigEndian(byte[] ain){
  if(ain.length%8!=0)throw new IllegalArgumentException("Length is not multiple of 8");
  long[] aout=new long[ain.length/8];
  for(int i=0;i<ain.length;i++)aout[i/8]|=(ain[i]&0xFFL)<<((7-i%8)*8);
  return aout;}

 public static long[] fromBytesLittleEndian(byte[] ain){
  if(ain.length%8!=0)throw new IllegalArgumentException("Length is not multiple of 8");
  long[] aout=new long[ain.length/8];
  for(int i=0;i<ain.length;i++)aout[i/8]|=(ain[i]&0xFFL)<<(i%8*8);
  return aout;}
