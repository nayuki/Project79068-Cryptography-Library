 ...
 */
 public static int reciprocalMod(int x,int m){
  if(x==0)throw new IllegalArgumentException("Division by zero");
  if(m<2)throw new IllegalArgumentException("Reciprocal does not exist");
  int y=x;
  x=m;
  int a=0,b=1;
  while(true){ // Extended Euclidean algorithm
   int z=x%y;
   if(z==0){
    if(y==1)return mod(b,m); // GCD is 1; reciprocal exists
 ...
 */
 public static int powMod(int x,int y,int m){
  if(y<0)return powMod(reciprocalMod(x,m),-y,m);
  int z=1;
  for(;y!=0;y>>>=1,x=x*x%m){
   if((y&1)!=0)z=z*x%m;}
  return z;}
 ...
 <p>Sample values:</p>
 <ul><li><code>sqrt(4) = 2</code></li>
  <li><code>sqrt(5) = 2</code></li>
  <li><code>sqrt(9) = 3</code></li></ul>
 ...
 */
 ...
 <p>Sample values:</p>
 <ul><li><code>cbrt(1) = 1</code></li>
  <li><code>cbrt(5) = 1</code></li>
  <li><code>cbrt(8) = 2</code></li></ul>
 */
 public static int cbrt(int x){
  if(x==-2147483648)return -1290;
 ...
 */
 public static long factorial(int x){
  if(x<0)throw new IllegalArgumentException("Factorial of negative integer");
  if(x>20)throw new ArithmeticOverflowException(String.format("factorial(%d)",x));
  int p=1;
  for(;x>=2;x--)p*=x;
  return p;}
 ...
 */
 public static int totient(int x){
  if(x<1)throw new IllegalArgumentException("Totient of negative integer");
  int p=1;
 ...
 */
 public static boolean isPrime(int x){
  if(x<0)throw new IllegalArgumentException("Undefined for negative integers");
 ...
 */
 ...
 public static int divideAndFloor(int x,int y){
  if(x==-2147483648&&y==-1)throw new ArithmeticOverflowException(String.format("divideAndFloor(%d, %d)",x,y));
  else if((x>=0)==(y>=0))return x/y; // If they have the same sign
  else{
   int z=x/y;
   if(z*y==x)return z;
   else return z-1;}}
 ...
 */
 public static int fibonacci(int x){
  if(x<-46||x>46)throw new ArithmeticOverflowException(String.format("fibonacci(%d)",x));
  int a=1;
  int b=0;
  int c=1;
  if(x>=0){
 ...
 */
 public static int floorToPowerOf2(int x){
  if(x<=0)throw new IllegalArgumentException("Non-positive argument");
  x|=x>>> 1;
  x|=x>>> 2;
  x|=x>>> 4;
  x|=x>>> 8;
 ...
 */
 public static int ceilingToPowerOf2(int x){
  if(x<=0)throw new IllegalArgumentException("Non-positive argument");
  if(x>1073741824)throw new ArithmeticOverflowException(String.format("ceilingToPowerOf2(%d)",x));
  x--;
  x|=x>>> 1;
  x|=x>>> 2;
  x|=x>>> 4;
