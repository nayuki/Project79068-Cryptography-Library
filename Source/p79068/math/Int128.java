...
public final class Int128 implements Comparable<Int128>{
 ...
 public Int128 add(Int128 num){
  ...
  if(LongMath.compareUnsigned(newlow,low)<0)newhigh++; // Carry
 ...
 public Int128 subtract(Int128 num){
  long newlow=low-num.low;
  long newhigh=high-num.high;
  if(LongMath.compareUnsigned(newlow,low)>0)newhigh--; // Borrow
  return new Int128(newhigh,newlow);}
 ...
 public Int128 shiftLeft(int shift){
  ...
  else if(shift<64)return new Int128(high<<shift|low>>>(64-shift),low<<shift);
  else return new Int128(low<<(shift-64),0);}

 /** Returns <code>this >> shift</code>. <code>shift</code> is interpreted as its value modulo 128. */
 public Int128 shiftRight(int shift){
  shift&=0x7F;
  if(shift==0)return this;
  else if(shift<64)return new Int128(high>>shift,high<<(64-shift)|low>>>shift);
  else{
   if(high>=0)return new Int128( 0L,high>>(shift-64));
   else       return new Int128(-1L,high>>(shift-64));}}
 ...
 public Int128 shiftRightUnsigned(int shift){
  ...
  else return new Int128(0,high>>>(shift-64));}
 ...
 public int compareTo(Int128 num){
  if(high!=num.high)return LongMath.compare(high,num.high);
