...
public final class ArrayStack<E> extends Stack<E>{
 ...
 public ArrayStack(int initCapacity){
  if(initCapacity<1)throw new IllegalArgumentException("Initial capacity less than 1");
 ...
 public E pop(){
  ...
  data[top-1]=null;
  top--;
  if(top<=data.length/4&&data.length/2>=1)resize(data.length/2);
  return result;}
 ...
 private void resize(int newCapacity){
  if(newCapacity<top||newCapacity<1)throw new AssertionError();
