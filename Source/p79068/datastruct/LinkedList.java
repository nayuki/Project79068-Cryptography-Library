...
public final class LinkedList<E> extends List<E>{
 ...
 public int count(Object o){
  ...
  int count=0;
  for(LinkedListNode<E> node=getNode(0);node!=null;node=node.next){
   if(o.equals(node.value))count++;}
  return count;}


 public void append(E obj){
  NullChecker.check(obj);
  if(length==Integer.MAX_VALUE)throw new IllegalStateException("List has reached maximum length (2147483647)");
 ...
 public void insert(int index,E obj){
  NullChecker.check(obj);
  if(index<0||index>length)throw new IndexOutOfBoundsException(String.format("Bounds = [0,%d), insertion index = %d",length,index));
  if(length==Integer.MAX_VALUE)throw new IllegalStateException("List has reached maximum length (2147483647)");
 ...
 public void clearTo(E obj){
  for(LinkedListNode<E> node=getNode(0);node!=null;node=node.next)node.value=obj;}
 ...
 public int findNext(Object o,int start){
  ...
  for(;node!=null;node=node.next,index++){
   if(o.equals(node.value))return index;}
  return -1;}
 ...
 public int findPrevious(Object o,int start){
  ...
  for(;node!=null&&index<=start;node=node.next,index++){
   if(o.equals(node.value))result=index;}
  return result;}
 ...
 public boolean equals(Object obj){
  ...
  LinkedListNode<E> node=getNode(0);
  for(E listobj:list){
   if(!node.value.equals(listobj))return false;
   node=node.next;}
  return true;}
 ...
 public int hashCode(){
  ...
  h.add(length);
  for(LinkedListNode<E> node=getNode(0);node!=null;node=node.next)h.add(node.value.hashCode());
