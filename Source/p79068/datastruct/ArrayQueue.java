...
public final class ArrayQueue<E> extends Queue<E>{
 ...
 public ArrayQueue(int initCapacity){
  if(initCapacity<2)throw new IllegalArgumentException("Initial capacity less than 2");
 ...
 public void enqueue(E obj){
  ...
  if(length()+1==data.length)resize(data.length*2);
  data[tail]=obj;
  tail++;
 ...
 public E dequeue(){
  ...
  if(length()<=data.length/4&&data.length/2>=2)resize(data.length/2);
  return result;}
 ...
 public int length(){
  ...
  else return tail+data.length-head;}
 ...
 private void resize(int newCapacity){
  if(newCapacity<2)throw new AssertionError();
