...
public final class HashValue implements Comparable<HashValue>{
 ...
 HashValue(HashFunction hashFunc,byte[] hashVal){
  if(hashFunc==null||hashVal==null||hashVal.length!=hashFunc.getHashLength())throw new AssertionError();
 ...
 public String toHexString(){
  ...
  for(int i=0;i<hashValue.length;i++)sb.append(hexDigits[hashValue[i]>>>4&0xF]).append(hexDigits[hashValue[i]&0xF]);
  return sb.toString();}
 ...
 public int compareTo(HashValue hv){
  if(!hashFunction.equals(hv.hashFunction))throw new IllegalArgumentException("Hash functions are different");
  if(hashValue.length!=hv.hashValue.length)throw new AssertionError("Hash lengths are different");
  for(int i=0;i<hashValue.length&&i<hv.hashValue.length;i++){
   if(hashValue[i]!=hv.hashValue[i])return (hashValue[i]&0xFF)-(hv.hashValue[i]&0xFF);}
  return 0;}
