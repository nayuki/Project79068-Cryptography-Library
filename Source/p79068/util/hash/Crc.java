...
public class Crc extends HashFunction{
 ...
 public Crc(String name,int deg,long poly,boolean revin,boolean revout,long xorin,long xorout){
  ...
  if(deg<1||deg>64)throw new IllegalArgumentException("Invalid degree");
  if(deg<64){
   if((poly>>>deg)!=1)throw new IllegalArgumentException("Illegal polynomial");
   if((xorin>>>deg)!=0)throw new IllegalArgumentException("Invalid initial XOR constant");
   if((xorout>>>deg)!=0)throw new IllegalArgumentException("Invalid final XOR constant");}
  this.name=name;
  this.degree=deg;
 ...
 public Hasher newHasher(){
  if(equals(CRC32_FUNCTION))return new Crc32Hasher(this);
 ...
 public boolean equals(Object obj){
  ...
  if(!(obj instanceof Crc))return false;
  Crc hashfunc=(Crc)obj;
  return name.equals(hashfunc.name)
   &&degree    ==hashfunc.degree
   &&polynomial==hashfunc.polynomial
   &&reversein ==hashfunc.reversein
   &&reverseout==hashfunc.reverseout
   &&xorin     ==hashfunc.xorin
   &&xorout    ==hashfunc.xorout;}
