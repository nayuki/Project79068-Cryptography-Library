 ...
 */
 public int randomInt(int n){
  ...
  if(IntegerMath.isPowerOf2(n))return randomInt()&(n-1);
  else{ // Unbiased
   int random;
   int result;
   do{
 ...
 */
 public void randomBytes(byte[] b,int off,int len){
  ...
  int end=off+len;
  for(int temp=randomInt();off%4!=0;off++,temp>>>=8)b[off]=(byte)temp; // Fill until off is a multiple of 4, if necessary (less than 4 bytes)
  for(int end4=end/4*4;off<end4;off+=4){ // Fill 4 bytes at a time
   ...
   b[off|2]=(byte)(temp>>> 8);
   b[off|3]=(byte)(temp>>> 0);}
  for(int temp=randomInt();off<end;off++,temp>>>=8)b[off]=(byte)temp;} // Fill the last few bytes (less than 4)
