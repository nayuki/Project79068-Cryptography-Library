...
final class FastAesCipherer extends RijndaelCiphererParent{
 ...
 public void encrypt(byte[] b,int off,int len){
  ...
  if((len&0xF)!=0)throw new IllegalArgumentException("Invalid block length");
 ...
 private void setKey(byte[] key){
  if(key.length%4!=0||key.length==0)throw new IllegalArgumentException("Invalid key length");
  ...
  for(int i=0;i<roundCount+1;i++){
   if(i==0||i==roundCount)System.arraycopy(encryptionKeySch,(roundCount-i)*4,decryptionKeySch,i*4,4);
   else{
    for(int j=0;j<4;j++){
     decryptionKeySch[i*4+j]=
       mulinv0[sub[encryptionKeySch[(roundCount-i)*4+j]>>>24&0xFF]&0xFF]
      ^mulinv1[sub[encryptionKeySch[(roundCount-i)*4+j]>>>16&0xFF]&0xFF]
      ^mulinv2[sub[encryptionKeySch[(roundCount-i)*4+j]>>> 8&0xFF]&0xFF]
      ^mulinv3[sub[encryptionKeySch[(roundCount-i)*4+j]>>> 0&0xFF]&0xFF];}}}}
 ...
 private static void initMultiplyTable(){
  ...
  for(int i=0;i<256;i++){
   int j=sub[i]&0xFF;
   mul0[i]=multiply(j,0x02)<<24|multiply(j,0x01)<<16|multiply(j,0x01)<<8|multiply(j,0x03);
   mul1[i]=mul0[i]<<24|mul0[i]>>> 8;
   mul2[i]=mul0[i]<<16|mul0[i]>>>16;
   mul3[i]=mul0[i]<< 8|mul0[i]>>>24;
   j=subinv[i]&0xFF;
   mulinv0[i]=multiply(j,0x0E)<<24|multiply(j,0x09)<<16|multiply(j,0x0D)<<8|multiply(j,0x0B);
   mulinv1[i]=mulinv0[i]<<24|mulinv0[i]>>> 8;
   mulinv2[i]=mulinv0[i]<<16|mulinv0[i]>>>16;
