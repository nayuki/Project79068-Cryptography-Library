...
class RijndaelCipherer extends RijndaelCiphererParent{
 ...
 protected void setKey(byte[] key){
  ...
  for(int i=0;i<keySchedule.length;i++)keySchedule[i]=toBytesBigEndian(w,i*nb,nb);}
 ...
 protected void subBytes(byte[] block){
  for(int i=0;i<blockLength;i++)block[i]=sub[block[i]&0xFF];}
 ...
 protected void mixColumns(byte[] blockin,byte[] sout){
  for(int i=0;i<blockLength;i+=4){
   for(int j=0;j<4;j++)sout[i+j]=(byte)(mul02[blockin[i+j]&0xFF]^mul03[blockin[i+(j+1)%4]&0xFF]^blockin[i+(j+2)%4]^blockin[i+(j+3)%4]);}}

 protected void addRoundKey(byte[] block,byte[] key){ // Self-inverting
  for(int i=0;i<blockLength;i++)block[i]^=key[i];}

 protected void subBytesInverse(byte[] block){
  for(int i=0;i<blockLength;i++)block[i]=subinv[block[i]&0xFF];}
 ...
 protected void mixColumnsInverse(byte[] blockin,byte[] sout){
  for(int i=0;i<blockLength;i+=4){
   for(int j=0;j<4;j++)sout[i+j]=(byte)(mul0E[blockin[i+j]&0xFF]^mul0B[blockin[i+(j+1)%4]&0xFF]^mul0D[blockin[i+(j+2)%4]&0xFF]^mul09[blockin[i+(j+3)%4]&0xFF]);}}
