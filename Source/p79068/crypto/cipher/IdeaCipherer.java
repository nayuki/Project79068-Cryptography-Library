...
final class IdeaCipherer extends Cipherer{
 ...
 public void encrypt(byte[] b,int off,int len){
  ...
  if((len&0x7)!=0)throw new IllegalArgumentException("Invalid block length");
 ...
 public void zeroize(){
  ...
  for(int i=0;i<encKeySch.length;i++)encKeySch[i]=0;
  for(int i=0;i<decKeySch.length;i++)decKeySch[i]=0;
  encKeySch=null;
  decKeySch=null;
  super.zeroize();}
 ...
 private void setKey(byte[] key){
  ...
  for(int i=0;i<encKeySch.length;i++)encKeySch[i]=getInt16(key,i*16+i/8*25);
 ...
 private static int multiply(int x,int y){ // This has been verified by brute force. But it can be proven mathematically too.
  if(x==0)return (0x10001-y)&0xFFFF;
  if(y==0)return (0x10001-x)&0xFFFF;
  int z=x*y;
  z=(z&0xFFFF)-(z>>>16);
  if(z<0)z+=0x10001;
  return z&0xFFFF;}
 ...
 private static int getInt16(byte[] b,int bitOffset){ // Returns 16 consecutive bits from b starting at bitOffset (measured in bits), which is modulo the number of bits in b.
  int result=
    ((b[(bitOffset/8+0)%b.length]<<(8+bitOffset%8))
   |((b[(bitOffset/8+1)%b.length]&0xFF)<<(bitOffset%8))
   |((b[(bitOffset/8+2)%b.length]&0xFF)>>>(8-bitOffset%8)));
