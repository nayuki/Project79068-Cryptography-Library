...
class WhirlpoolCiphererParent extends Cipherer{
 ...
 WhirlpoolCiphererParent(Cipher cipher,byte[] key){
  super(cipher,key);
  if(!(cipher instanceof Whirlpool0Cipher)&&!(cipher instanceof Whirlpool1Cipher)&&!(cipher instanceof WhirlpoolCipher))throw new AssertionError();}


 public void encrypt(byte[] b,int off,int len){
  if(cipher==null)throw new IllegalStateException("Already zeroized");
  BoundsChecker.check(b.length,off,len);
  if((len&0x3F)!=0)throw new IllegalArgumentException();
 ...
 private void w(byte[] block,byte[] temp){ // The internal block cipher. Overwrites block and temp.
  ...
  for(int i=1;i<keySchedule.length;i++)rho(block,keySchedule[i],temp);}
 ...
 private void wInverse(byte[] block,byte[] temp){
  for(int i=keySchedule.length-1;i>=1;i--)rhoInverse(block,keySchedule[i],temp);
  sigma(block,keySchedule[0]);}
 ...
 private void gammaInverse(byte[] block){
  for(int i=0;i<64;i++)block[i]=subinv[block[i]&0xFF];}

 private void piInverse(byte[] blockin,byte[] blockout){
  for(int j=0;j<8;j++){
   for(int i=0;i<8;i++)blockout[i<<3|j]=blockin[((i+j)&7)<<3|j];}}

 private void thetaInverse(byte[] blockin,byte[] blockout){
  for(int i=0;i<8;i++){
   for(int j=0;j<8;j++){
    int sum=0;
    for(int k=0;k<8;k++)sum^=mulinv[k][blockin[i<<3|(j+k)&7]&0xFF];
    blockout[i<<3|j]=(byte)sum;}}}
