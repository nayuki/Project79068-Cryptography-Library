...
final class AesCipherer extends RijndaelCipherer{
 ...
 AesCipherer(Rijndael cipher,byte[] key){
  super(cipher,key);
  if(cipher.getBlockLength()!=16)throw new AssertionError();
  setKey(key);}
 ...
 protected void subBytes(byte[] block){
  for(int i=0;i<16;i++)block[i]=sub[block[i]&0xFF];}

 protected void shiftRows(byte[] blockin,byte[] blockout){
  for(int i=0;i<4;i++){
   for(int j=0;j<4;j++)blockout[i+j*4]=blockin[i+(j+i)%4*4];}}

 protected void mixColumns(byte[] blockin,byte[] blockout){
  for(int i=0;i<16;i+=4){
   for(int j=0;j<4;j++)blockout[i+j]=(byte)(mul02[blockin[i+j]&0xFF]^mul03[blockin[i+(j+1)%4]&0xFF]^blockin[i+(j+2)%4]^blockin[i+(j+3)%4]);}}
 ...
 protected void subByteblockinverse(byte[] block){
  for(int i=0;i<16;i++)block[i]=subinv[block[i]&0xFF];}

 protected void shiftRowblockinverse(byte[] blockin,byte[] blockout){
  for(int i=0;i<4;i++){
   for(int j=0;j<4;j++)blockout[i+j*4]=blockin[i+(j-i+4)%4*4];}}

 protected void mixColumnblockinverse(byte[] blockin,byte[] blockout){
  for(int i=0;i<16;i+=4){
   for(int j=0;j<4;j++)blockout[i+j]=(byte)(mul0E[blockin[i+j]&0xFF]^mul0B[blockin[i+(j+1)%4]&0xFF]^mul0D[blockin[i+(j+2)%4]&0xFF]^mul09[blockin[i+(j+3)%4]&0xFF]);}}}