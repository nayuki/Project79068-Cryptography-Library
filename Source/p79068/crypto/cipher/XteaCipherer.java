...
final class XteaCipherer extends Cipherer{
 ...
 XteaCipherer(Xtea cipher,byte[] key){
  super(cipher,key);
  int32Key=new int[4];
  for(int i=0;i<int32Key.length;i++)int32Key[i]=key[i*4]<<24|(key[i*4+1]&0xFF)<<16|(key[i*4+2]&0xFF)<<8|(key[i*4+3]&0xFF);}


 public void encrypt(byte[] b,int off,int len){
  if(cipher==null)throw new IllegalStateException("Already zeroized");
  BoundsChecker.check(b.length,off,len);
  if((len&7)!=0)throw new IllegalArgumentException("Invalid block length");
