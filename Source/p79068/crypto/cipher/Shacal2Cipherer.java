 ...
 */
 public void encrypt(byte[] B,int off,int len){
  ...
  if((len&0x1F)!=0)throw new IllegalArgumentException("Invalid block length");
  for(len+=off;off<len;off+=32){
 ...
 */
 ...
 private void setKey(byte[] key){ // If the key is shorter than 64 bytes, then zeros are appended
  ...
  for(;i<64;i++)keySchedule[i]=keySchedule[i-16]+((keySchedule[i-15]<<25|keySchedule[i-15]>>>7)^(keySchedule[i-15]<<14|keySchedule[i-15]>>>18)^(keySchedule[i-15]>>>3))+keySchedule[i-7]+((keySchedule[i-2]<<15|keySchedule[i-2]>>>17)^(keySchedule[i-2]<<13|keySchedule[i-2]>>>19)^(keySchedule[i-2]>>>10));}
