package p79068.crypto.cipher;


final class Whirlpool0Cipherer extends WhirlpoolCiphererParent{

 Whirlpool0Cipherer(Whirlpool0Cipher cipher,byte[] key){
  super(cipher,key);
  sub=WHIRLPOOL0_SUB;
  subinv=WHIRLPOOL0_SUBINV;
  mul=WHIRLPOOL0_MUL;
  mulinv=WHIRLPOOL0_MULINV;
  rcon=WHIRLPOOL0_RCON;
  setKey(key);}


 private static final byte[] WHIRLPOOL0_SUB;
 private static final byte[] WHIRLPOOL0_SUBINV;
 private static final byte[][] WHIRLPOOL0_MUL;
 private static final byte[][] WHIRLPOOL0_MULINV;
 private static final byte[][] WHIRLPOOL0_RCON;

 static{
  {int[] sub={
    };
   WHIRLPOOL0_SUB=new byte[sub.length];
   WHIRLPOOL0_SUBINV=new byte[sub.length];
   for(int i=0;i<sub.length;i++){
    WHIRLPOOL0_SUB[i]=(byte)sub[i];
    WHIRLPOOL0_SUBINV[sub[i]]=(byte)i;}}

  {WHIRLPOOL0_RCON=new byte[ROUNDS][64];
   for(int i=0;i<WHIRLPOOL0_RCON.length;i++){
    int j=0;
    for(;j<8;j++)WHIRLPOOL0_RCON[i][j]=WHIRLPOOL0_SUB[8*i+j];
    for(;j<64;j++)WHIRLPOOL0_RCON[i][j]=0;}}

  {int[] c   ={0x01,0x05,0x09,0x08,0x05,0x01,0x03,0x01};
   int[] cinv={0xB8,0x0E,0x67,0x6C,0x0A,0xD1,0x71,0xE3};
   WHIRLPOOL0_MUL=new byte[8][256];
   WHIRLPOOL0_MULINV=new byte[8][256];
   for(int i=0;i<c.length;i++){
    for(int j=0;j<256;j++){
     WHIRLPOOL0_MUL[i][j]=(byte)multiply(j,c[i]);
     WHIRLPOOL0_MULINV[i][j]=(byte)multiply(j,cinv[i]);}}}}}