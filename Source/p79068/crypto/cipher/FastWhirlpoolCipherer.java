...
final class FastWhirlpoolCipherer extends Cipherer{
 ...
 public void encrypt(byte[] b,int off,int len){
  ...
  if((len&0x3F)!=0)throw new IllegalArgumentException("Invalid block length");
...
//   wInverse(tempmsg,temp);
 ...
 private void w(long[] block,long[] temp){ // The internal block cipher. Overwrites block and temp.
  for(int i=0;i<8;i++)block[i]^=keySchedule[0][i]; // Sigma
  for(int i=0;i<rcon.length;i++)rho(block,keySchedule[i],temp);}
