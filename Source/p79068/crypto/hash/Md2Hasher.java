package p79068.crypto.hash;

import p79068.util.hash.HashValue;


final class Md2Hasher extends BlockHasher{

 private int[] state;
 private int[] checksum;


 Md2Hasher(Md2 algor){
  super(algor,16);
  state=new int[48];
  checksum=new int[16];}


 public Md2Hasher clone(){
  if(hashFunction==null)throw new IllegalStateException("Already zeroized");
  Md2Hasher result=(Md2Hasher)super.clone();
  result.state=(int[])state.clone();
  result.checksum=(int[])checksum.clone();
  return result;}


 public void zeroize(){
  if(hashFunction==null)throw new IllegalStateException("Already zeroized");
  for(int i=0;i<state.length;i++)state[i]=0;
  for(int i=0;i<checksum.length;i++)checksum[i]=0;
  state=null;
  checksum=null;
  super.zeroize();}


 private static final int[] s={ // Substitution table derived from pi. This is a permutation.
  };

 protected void compress(byte[] message,int off,int len){
  for(len+=off;off<len;off+=16){
   for(int i=0;i<16;i++)state[32|i]=(state[16|i]=message[off+i]&0xFF)^state[i];
   int t=0;
   for(int i=0;i<18;i++){
    for(int j=0;j<48;j++)t=state[j]^=s[t];
    t=(t+i)&0xFF;}
   t=checksum[15]; // This variable is reused as "l"
   for(int i=0;i<16;i++)t=checksum[i]^=s[(message[off+i]&0xFF)^t];}}

 protected HashValue getHashDestructively(){
  for(int i=blockLength;i<block.length;i++)block[i]=(byte)(16-blockLength);
  compress();
  for(int i=0;i<16;i++)block[i]=(byte)checksum[i];
  compress();
  byte[] hash=new byte[16];
  for(int i=0;i<16;i++)hash[i]=(byte)state[i];
  return createHash(hash);}}