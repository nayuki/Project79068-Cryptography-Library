...
final class Md5Hasher extends BlockHasher{
 ...
 protected void compress(byte[] message,int off,int len){
  ...
  for(len+=off;off<len;){ // len becomes end
   for(int i=0;i<16;i++,off+=4)schedule[i]=(message[off]&0xFF)|(message[off+1]&0xFF)<<8|(message[off+2]&0xFF)<<16|message[off+3]<<24;
   int i=0;
   for(;i<16;i++){
 ...
 protected HashValue getHashDestructively(){
  ...
  for(int i=0;i<8;i++)block[block.length-8+i]=(byte)((length*8)>>>(i*8));
  compress();
