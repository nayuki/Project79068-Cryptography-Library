 ...
 */
 public Hmac(BlockHashFunction hashFunc,byte[] key){
  if(hashFunc.getBlockLength()<hashFunc.getHashLength())throw new IllegalArgumentException();
  inner=hashFunc.newHasher();
  outer=hashFunc.newHasher();
  key=key.clone();
  key=preprocessKey(hashFunc,key);
  for(int i=0;i<key.length;i++)key[i]^=0x36;
  inner.update(key);
  for(int i=0;i<key.length;i++)key[i]^=0x6A; // Un-XOR by 0x36, then XOR by 0x5C
  outer.update(key);}
 ...
 */
 public Hasher newHasher(){
  if(outer==null)throw new IllegalStateException("Already zeroized");
 ...
 */
 public void zeroize(){
  if(outer==null)throw new IllegalStateException("Already zeroized");
  if(inner instanceof Zeroizable)((Zeroizable)inner).zeroize();
  if(outer instanceof Zeroizable)((Zeroizable)outer).zeroize();
  inner=null;
  outer=null;}


 private static byte[] preprocessKey(BlockHashFunction hashFunc,byte[] key){
  if(key.length>hashFunc.getBlockLength())key=hashFunc.getHash(key).toBytes();
